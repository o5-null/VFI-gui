"""
ATM-VFI Network (lite variant).

Hidden dims [16, 32, 64, 96] with reduced MLP ratio for lighter inference.
...
class Network(nn.Module):
    """ATM-VFI lite network with 4 pyramid levels and hidden dims [16, 32, 64, 96]."""
    ...
    def __init__(self, global_motion=True, ensemble_global_motion=False):
        ...
        for i in range(self.pyramid_level):
            if i == 0:
                self.feat_extracts.append(
                    nn.Sequential(conv(3, self.hidden_dims[i]), conv(self.hidden_dims[i], self.hidden_dims[i]))
                )
            else:
                self.feat_extracts.append(
                    nn.Sequential(conv(self.hidden_dims[i - 1], self.hidden_dims[i], kernel_size=3, stride=2, padding=1), conv(self.hidden_dims[i], self.hidden_dims[i]))
                )
        ...
        self.local_motion_args = {"window_size": 8, "num_heads": 8, "patch_size": 1, "dim": fused_dim, "enhance_window": 8, "mlp_ratio": 2}

        self.feat_enhance_transformer = nn.ModuleList([
            SwinTransformer(dim=self.local_motion_args["dim"], window_size=self.local_motion_args["enhance_window"], shift_size=0, patch_size=self.local_motion_args["patch_size"], num_heads=self.local_motion_args["num_heads"], mlp_ratio=self.local_motion_args["mlp_ratio"]),
            SwinTransformer(dim=self.local_motion_args["dim"], window_size=self.local_motion_args["enhance_window"], shift_size=self.local_motion_args["enhance_window"] // 2, patch_size=self.local_motion_args["patch_size"], num_heads=self.local_motion_args["num_heads"], mlp_ratio=self.local_motion_args["mlp_ratio"]),
        ])

        self.local_motion_atmformer = nn.ModuleList([
            ATMFormer(dim=self.local_motion_args["dim"], window_size=self.local_motion_args["window_size"], shift_size=0, patch_size=self.local_motion_args["patch_size"], num_heads=self.local_motion_args["num_heads"], mlp_ratio=self.local_motion_args["mlp_ratio"]),
            ATMFormer(dim=self.local_motion_args["dim"], window_size=self.local_motion_args["window_size"], shift_size=self.local_motion_args["window_size"] // 2, patch_size=self.local_motion_args["patch_size"], num_heads=self.local_motion_args["num_heads"], mlp_ratio=self.local_motion_args["mlp_ratio"]),
        ])
        ...
        # Global motion
        last_feat_dim = self.hidden_dims[-1] + 32
        ...
        self.global_motion_args = {"window_size": 12, "num_heads": 8, "patch_size": 1, "dim": concat_dim, "mlp_ratio": 2}

        self.global_motion_atmformer = nn.ModuleList([
            ATMFormer(dim=self.global_motion_args["dim"], window_size=self.global_motion_args["window_size"], shift_size=0, patch_size=self.global_motion_args["patch_size"], num_heads=self.global_motion_args["num_heads"], mlp_ratio=self.global_motion_args["mlp_ratio"]),
            ATMFormer(dim=self.global_motion_args["dim"], window_size=self.global_motion_args["window_size"], shift_size=self.global_motion_args["window_size"] // 2, patch_size=self.global_motion_args["patch_size"], num_heads=self.global_motion_args["num_heads"], mlp_ratio=self.global_motion_args["mlp_ratio"]),
        ])
        ...
        self.upsample_pyramid = nn.ModuleList([
            nn.Sequential(
                ...
                conv(self.fused_dim1 + self.motion_out_dim, self.fused_dim1 + self.motion_out_dim),
                nn.Conv2d(self.fused_dim1 + self.motion_out_dim, self.fused_dim1 + self.motion_out_dim, 3, 1, 1),
            ),
            nn.Sequential(
                ...
                conv(self.fused_dim2 + self.motion_out_dim, self.fused_dim2 + self.motion_out_dim),
                nn.Conv2d(self.fused_dim2 + self.motion_out_dim, self.fused_dim2 + self.motion_out_dim, 3, 1, 1),
            ),
            nn.Sequential(
                ...
                conv(self.fused_dim3 + self.motion_out_dim, self.fused_dim3 + self.motion_out_dim),
                nn.Conv2d(self.fused_dim3 + self.motion_out_dim, self.fused_dim3 + self.motion_out_dim, 3, 1, 1),
            ),
        ])
        ...
        hidden_dim = 32
        self.proj_ref = conv(in_chan, hidden_dim)
        ...
        self.up2 = nn.Sequential(
            deconv(4 * hidden_dim, 2 * hidden_dim, kernel_size=2, stride=2, padding=0),
            conv(2 * hidden_dim, 1 * hidden_dim),
        )
        self.up3 = nn.Sequential(deconv(2 * hidden_dim, 1 * hidden_dim, kernel_size=2, stride=2, padding=0))
    ...
    def _estimate_local_motion(self, feat):
        ...
        return out[:, :2], out[:, 2:4], torch.sigmoid(out[:, 4].unsqueeze(1)), feat, out
    ...
    def _residual_refinement(self, feat, im0, I_t_0, im1, I_t_1, I_t, backbone_decoder_feats):
        ...
        return 2 * torch.sigmoid(I_t_residual) - 1
    ...
    def _forward_normal(self, im0, im1):
        ...
        if self.global_motion:
            ...
            opt_flow_0_up = upsample_flow(opt_flow_0, 2, "bilinear")
            opt_flow_1_up = upsample_flow(opt_flow_1, 2, "bilinear")
            ...
            for i in reversed(range(self.pyramid_level)):
                ...
                if i != 0:
                    opt_flow_0_up = upsample_flow(opt_flow_0_up, 2, "bilinear")
                    opt_flow_1_up = upsample_flow(opt_flow_1_up, 2, "bilinear")
        else:
            feat = einops.rearrange(feat, "B (H W) C -> B H W C", H=h)
    ...
    def _forward_global_ensemble(self, im0, im1):
        # Same logic as _forward_normal but with multiscale_global_motion_ensemble
        B, _, H, W = im0.size()
        im0_list = [im0]
        im1_list = [im1]
        for scale in range(self.pyramid_level - 1):
    ...
    def _multiscale_global_motion_ensemble(self, im0, im1):
        ...
        for i in range(B):
            ...
            if scale_idx == 0:
                ...
                opt_flow0[i] = upsample_flow(level1[0][i, None], 2, "bilinear")
                opt_flow1[i] = upsample_flow(level1[1][i, None], 2, "bilinear")
            else:
                opt_flow0[i] = upsample_flow(level2[0][i, None], 4, "bilinear")
                opt_flow1[i] = upsample_flow(level2[1][i, None], 4, "bilinear")
    ...
    def _global_alignmentness(self, x, im0, im1):
        ...
        _, _, H0, W0 = im0.size()
        factor = H0 // H1
        opt_flow0 = upsample_flow(opt_flow0, factor, "bilinear")
        opt_flow1 = upsample_flow(opt_flow1, factor, "bilinear")
        im0 = flow_warp(im0, opt_flow0)
        im1 = flow_warp(im1, opt_flow1)
        return F.l1_loss(im0, im1, reduction="none").mean(dim=[1, 2, 3])
