"""
ATM (Attention to Motion) — attention-based optical flow estimation.
...
def pad_if_needed(x, size, window_size):
    ...
    if pad_h > 0 or pad_w > 0:
        img_mask = torch.zeros((1, h + pad_h, w + pad_w, 1))
        h_slices = (slice(0, pad_h // 2), slice(pad_h // 2, h + pad_h // 2), slice(h + pad_h // 2, None))
        w_slices = (slice(0, pad_w // 2), slice(pad_w // 2, w + pad_w // 2), slice(w + pad_w // 2, None))
        cnt = 0
        for h in h_slices:
            for w in w_slices:
                img_mask[:, h, w, :] = cnt
        ...
        return F.pad(x, (0, 0, pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2)), attn_mask
    return x, None
...
class AttentionToMotion(nn.Module):
    """Attention-to-Motion: extracts bidirectional motion from cross-frame attention."""

    def __init__(self, dim, window_size, patch_size=1, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.0, proj_drop=0.0):
        super().__init__()
    ...
    def _set_window_size_(self, window_size):
        ...
        self.relative_coord = self.relative_coord.to(device)
...
class ATMFormer(nn.Module):
    """ATM Transformer block with window-based attention-to-motion."""

    def __init__(self, dim, window_size=7, shift_size=0, patch_size=1, num_heads=8, mlp_ratio=4.0, bidirectional=True,
                 qkv_bias=False, qk_scale=None, drop=0.0, attn_drop=0.0, drop_path=0.0,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.window_size = to_2tuple(window_size) if not isinstance(window_size, (tuple, list)) else window_size
        self.shift_size = to_2tuple(shift_size) if not isinstance(shift_size, (tuple, list)) else shift_size
        self.bidirectional = bidirectional
        self.norm1 = norm_layer(dim)
        self.attn = AttentionToMotion(dim, window_size, patch_size=patch_size, num_heads=num_heads,
    ...
    def forward(self, x, H, W, B):
        """x: [2*B, H*W, C], frame1 and frame2 stacked in dim 0"""
        ...
        if self.shift_size[0] or self.shift_size[1]:
            ...
            if hasattr(self, "HW") and self.HW.item() == H_p * W_p:
                ...
                shift_mask = torch.zeros((1, H_p, W_p, 1))
...
class WindowAttention(nn.Module):
    """Standard window-based self-attention (Swin-like)."""
...
class RefineBottleneck(nn.Module):
    """Swin Transformer block used as feature refinement bottleneck."""

    def __init__(self, dim, window_size=8, shift_size=0, patch_size=1, num_heads=8, mlp_ratio=4.0, bidirectional=True,
                 qkv_bias=False, qk_scale=None, drop=0.0, attn_drop=0.0, drop_path=0.0,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.window_size = to_2tuple(window_size) if not isinstance(window_size, (tuple, list)) else window_size
        ...
        self.bidirectional = bidirectional
        self.norm1 = norm_layer(dim)
        self.attn = WindowAttention(dim, window_size, patch_size=patch_size, num_heads=num_heads,
                                    qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
