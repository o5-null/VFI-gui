...
class FILMModel(PyTorchVFIModel):
    """FILM model wrapper."""
    ...
    def load_model(self, checkpoint_path: str, **kwargs) -> None:
        ...
        if isinstance(loaded, torch.jit.ScriptModule):
            ...
            # Apply FP16 conversion if configured
    ...
    def interpolate(
        ...
        **kwargs
        ...
        if frame0.dtype != self.torch_dtype:
            ...
            frame1 = frame1.to(dtype=self.torch_dtype)
        
        # Pad to 64-pixel alignment (required by FILM model)
    ...
    def interpolate_multi(
        ...
        **kwargs
        ...
        Returns:
            Stacked interpolated frames tensor
        """
        frames = []
        timesteps = torch.linspace(1, num_frames, num_frames) / (num_frames + 1)
        
        for t in timesteps:
            frame = self.interpolate(frame0, frame1, t.item())
            frames.append(frame)
