"""
GMFSS Fortuna Union Architecture (with IFNet guidance).

Port from:
  https://github.com/98mxr/GMFSS_Fortuna
  https://github.com/ComfyUI-Frame-Interpolation

Sub-networks:
  - GMFlow: Global matching optical flow (transformer)
  - FeatureNet: Multi-scale feature extraction
  - MetricNet: Flow confidence estimation
  - FusionNet (GridNet): Frame blending
  - IFNet (RIFE-based): Flow initialization
"""
...
from ...rife.arch.ifnet import IFNet
...
backwarp_tenGrid2: dict = {}


def backwarp(tenIn, tenflow):
    if str(tenflow.shape) not in backwarp_tenGrid2:
        tenHor = (
            torch.linspace(
                start=-1.0,
                end=1.0,
        ...
        backwarp_tenGrid2[str(tenflow.shape)] = torch.cat([tenHor, tenVer], 1).to(tenflow.device)
    ...
    return torch.nn.functional.grid_sample(
        input=tenIn,
        grid=(backwarp_tenGrid2[str(tenflow.shape)] + tenflow).permute(0, 2, 3, 1),
        mode="bilinear",
        padding_mode="zeros",
        align_corners=True,
    )
...
class GridNet(nn.Module):
    ...
    def forward(self, x, x1, x2, x3):
        X00 = self.residual_model_head0(x) + self.residual_model_head1(x1)

        X01 = self.residual_model_01(X00) + X00

        X10 = self.downsample_model_10(X00) + self.residual_model_head2(x2)
        X20 = self.downsample_model_20(X10) + self.residual_model_head3(x3)

        residual_11 = self.residual_model_11(X10) + X10
        downsample_11 = self.downsample_model_11(X01)
        X11 = residual_11 + downsample_11

        residual_21 = self.residual_model_21(X20) + X20
        downsample_21 = self.downsample_model_21(X11)
        X21 = residual_21 + downsample_21

        X24 = self.residual_model_24(X21) + X21
        X25 = self.residual_model_25(X24) + X24

        upsample_14 = self.upsample_model_14(X24)
        residual_14 = self.residual_model_14(X11) + X11
        X14 = upsample_14 + residual_14

        upsample_04 = self.upsample_model_04(X14)
        residual_04 = self.residual_model_04(X01) + X01
        X04 = upsample_04 + residual_04

        upsample_15 = self.upsample_model_15(X25)
        residual_15 = self.residual_model_15(X14) + X14
        X15 = upsample_15 + residual_15

        upsample_05 = self.upsample_model_05(X15)
        residual_05 = self.residual_model_05(X04) + X04
        X05 = upsample_05 + residual_05

        X_tail = self.residual_model_tail(X05)

        return X_tail


class Model:
    """GMFSS Fortuna Union (with IFNet guidance)."""
    ...
    def load_model(self, path_dict):
        self.ifnet.load_state_dict(torch.load(path_dict["ifnet"], map_location="cpu", weights_only=False))
